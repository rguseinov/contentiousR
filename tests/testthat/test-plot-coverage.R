...
test_that("plot_coverage errors informatively when drop_empty removes every state", {
  skip_if_not_installed("ggplot2")
  ...
  expect_error(plot_coverage(panel, "x", drop_empty = TRUE), "No state has an event")
})
