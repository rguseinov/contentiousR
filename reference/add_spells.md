...
bundled column names.

## Usage

``` r
add_spells(panel, event = NULL, ongoing = NULL)
```
...
- event:
  ...
  `NULL` (the default), `add_spells()` looks for exactly one column in
  `panel` whose name ends in `_onset`, `_incidence`, or `_ongoing`; if
  zero or more than one match, it errors and asks for an explicit
  `event`.
