...
One can argue that `contentiousR` bundles several campaign/episode
datasets that all aim to record the same broad phenomenon — mass
anti-regime contention — but differ in coding rules, sourcing, and
scope. Guseinov, Ustyuzhanin, and Korotayev (2026) compare such kinds of
datasets and find that, while not conceptually identical, the major
sources are empirically highly convergent in their regional and temporal
...
region, so datasets of very different total size remain comparable. Pass
...
\
[`plot_regional_coverage`](https://rguseinov.github.io/contentiousR/reference/plot_regional_coverage.md)`(`\
`  datasets      ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"navco1.3"``, ``"navco2.1"``, ``"beissinger"``, ``"csra"``, ``"mec"``)``,`\
`  start_year    ``=`` ``1950``,`\
`  end_year      ``=`` ``2013``,`\
`  coding_system ``=`` ``"cow"`\
`)`
...
MEC sits well above the other four here. This is expected: MEC’s
underlying definition of a contentious episode is broader — reformist as
well as maximalist claims — than the “revolution” concept the other
datasets target, so it picks up substantially more episodes across every
period. (NAVCO 2.1 is compared here at the same campaign-onset level as
the other datasets).
...
`"csra"`, `"mec"`), so a narrower comparison is also possible:

\
[`plot_regional_coverage`](https://rguseinov.github.io/contentiousR/reference/plot_regional_coverage.md)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"navco1.3"``, ``"navco2.1"``)``, ``1950``, ``2013``, metric ``=`` ``"count"``)`
