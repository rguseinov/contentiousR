...
* `load_vdem_data(coding_system = "cow")` no longer silently drops East
  ...
  `"cow"` branch, so V-Dem coverage for Germany differed depending on the
  chosen coding system with no indication why.
* `add_spell_duration()` now computes spells from the actual calendar-year
  difference instead of row position within a state. A gap in the input
  years (e.g. left by `tidyr::drop_na(event)`, which the package's own
  examples recommend) previously understated the elapsed time.
* `plot_coverage()` now errors with a clear message instead of crashing
  when `drop_empty = TRUE` removes every state (e.g. the chosen event
  never occurs in the supplied panel/year range).
* `cn_cite()` matched keywords by substring, so a short `dataset` name
  (e.g. `"mm"`) could also match an unrelated entry whose keyword merely
  contained it as a substring (`"mmad"`). Keyword matching is now exact
  against the comma-separated keyword list.
* `harmonize_conflict_data()`: a factor `year_col` was coerced with
  `as.integer()`, which returns the factor's internal level codes rather
  than the printed year. It is now converted to character first. A
  `date_col` that fails to parse (e.g. a non-ISO format without a matching
  `date_format`) now raises a clear error instead of a generic base-R one.

## Bring your own data

* Added `harmonize_conflict_data()` to join externally supplied conflict/
  event data (GDELT, ICEWS, ACLED, or anything else not bundled with
  `contentiousR`) onto a state panel: it maps the caller's country column
  to COW/GW codes via `countrycode::countrycode()`, extracts a year from
  either a pre-parsed `year_col` or a `date_col`/`date_format`, optionally
  aggregates event-level rows to country-year (`<prefix>_onset`,
  `<prefix>_n_events`), and joins the result in. Unlike `add_conflict()`,
...
* Added formal `References` sections to `load_gdp_data()`/`add_gdp()`,
  `load_population_data()`/`add_population()`,
  `load_military_expenditure_data()`/`add_military_expenditure()`,
  `load_vdem_data()`/`add_vdem()`, `conflict_data()`/`add_conflict()`,
  `load_leader_data()`/`add_leader_data()`, and
  `plot_regional_coverage()`/`plot_temporal_coverage()`, citing the
  underlying data source(s) for each.
* Corrected the SCAD citation's DOI (in `DESCRIPTION` and the bundled
  bibliography behind `cn_cite()`): the previous DOI did not resolve.
