...
#' \url{https://ksgleditsch.com/archigos.html}
#'
...
load_leader_data <- function(
  ...
  if (dataset == "archigos") {
    path <- system.file("extdata", "archigos_4.1.txt.gz", package = "contentiousR")
    if (path == "") {
      stop("Could not find `archigos_4.1.txt.gz` in package extdata.", call. = FALSE)
    }
  ...
  if (dataset == "reign") {
    path <- system.file("extdata", "reign_leader_list.csv.gz", package = "contentiousR")
    if (path == "") {
      stop("Could not find `reign_leader_list.csv.gz` in package extdata.", call. = FALSE)
    }
